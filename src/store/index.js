...
不再需要通过import手动一个一个引入
  ...
  const context = require.context(
    处理的目录
    要不要递归遍历 (如果是true, 就会处理所有目录中的目录，如果是false,就只处理一层目录),
    处理的文件
    )
    ...
    context.keys() 获取到处理目录的所有文件路径列表

*/
// 处理./modules目录 (子目录不能处理)的下的所有js文件
const context = require.context('./modules', false, /\.js$/);
// console.log("context",context); 是一个函数
// context.keys() 函数当做对象使用: keys()
...
const modules = context.keys().reduce((modules, modulePath) => {
  ...
  // const m = context(模块路径) 相当于 import * as m from 模块路口
